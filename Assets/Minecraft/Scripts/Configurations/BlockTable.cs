...
namespace Minecraft.Scripts.Configurations
{
  ...
  public class BlockTable : ScriptableObject, IDisposable, ILuaCallCSharp
  {
    ...
    private AssetPtr m_BlockTableJson;

    [SerializeField] [EnsureAssetType(typeof(TextAsset))]
    private AssetPtr m_BlockMeshTableJson;

    [SerializeField] [EnsureAssetType(typeof(TextAsset))]
    private AssetPtr m_BlockTextureTableJson;

    [SerializeField] [EnsureAssetType(typeof(TextAsset))]
    private AssetPtr m_BlockMaterialTableJson;
    ...
    public void LoadBlockBehavioursInLua(IWorld world)
    {
      ...
      {
        ref var behaviour = ref m_BlockBehaviors[i];
    ...
    {
      var json = AssetManager.Instance.LoadAsset<TextAsset>(m_BlockTableJson);
      yield return json;
    ...
    {
      var json = AssetManager.Instance.LoadAsset<TextAsset>(m_BlockMeshTableJson);
      yield return json;
      ...
      var meshes = AssetManager.Instance.LoadAssets<BlockMesh>(m_BlockMeshPtrs);
      yield return AsyncAsset.WaitAll(m_BlockMeshes, meshes);
    }

    private IEnumerator InitTextures()
    {
      var json = AssetManager.Instance.LoadAsset<TextAsset>(m_BlockTextureTableJson);
      yield return json;

      m_TexturePtrs = JsonConvert.DeserializeObject<AssetPtr[]>(json.GetAssetAs<TextAsset>().text);

      var firstTexAsset = AssetManager.Instance.LoadAsset<Texture2D>(m_TexturePtrs[0]);
      yield return firstTexAsset;

      var firstTex = firstTexAsset.GetAssetAs<Texture2D>();
      ...
      for (var i = 0; i < m_TexturePtrs.Length; i++)
      {
        var texture = AssetManager.Instance.LoadAsset<Texture2D>(m_TexturePtrs[i]);
    ...
    {
      var json = AssetManager.Instance.LoadAsset<TextAsset>(m_BlockMaterialTableJson);
      yield return json;
      ...
      var materials = AssetManager.Instance.LoadAssets<Material>(m_MaterialPtrs);
