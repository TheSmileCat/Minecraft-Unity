...
using Minecraft.Scripts.ScriptableWorldGeneration.GenLayers.DefaultLayers;
using UnityEngine;
...
namespace Minecraft.Scripts.ScriptableWorldGeneration
{
  ...
  public class WorldGeneratePipeline : ScriptableObject, ILuaCallCSharp
  {
    ...
    [Space] [SerializeField] protected StatelessGenerator m_TerrainGenerator;
    [SerializeField] protected StatelessGenerator m_CaveGenerator;

    [SerializeField] [FormerlySerializedAs("m_MineGenerators")]
    protected StatelessGenerator[] m_ExtraGenerators;
    ...
    protected virtual StatelessGenLayer CreateGenLayers(int seed)
    {
      ...
      var zoomed2 = ZoomLayer.Magnify(seed, addIsland2, 4);
      ...
      var result = ZoomLayer.Magnify(seed, addRiver, 2);
    ...
    {
      var context = GetGenerationContext();
      chunk.GetRawDataNoCheck(out var transform, out var world, out var blocks, out var rotations, out var skyLights,
        out var heightMap);
      ...
      for (var i = 0; i < m_ExtraGenerators.Length; i++)
      {
    ...
    {
      var len0 = rotations.GetLength(0);
      var len1 = rotations.GetLength(1);
      var len2 = rotations.GetLength(2);
    ...
    {
      if (!m_GenContextPool.TryTake(out var context))
      {
