...
namespace Minecraft.Scripts.ScriptableWorldGeneration
{
  ...
  public class TerrainGenerator : StatelessGenerator
  {
    ...
    public Vector3 DepthNoiseScale = new(200f, 200f, 200f);
    public Vector3 MainNoiseScale = new(1 / 80f, 1 / 160f, 1 / 80f);
    ...
    {
      // 生物群系生成
      var biomeIds = helper.GenLayers.GetInts(pos.X - 8, pos.Z - 8, 32, 32, Allocator.TempJob);
      ...
      for (var i = 0; i < 10; i++)
      {
        ...
        {
          context.Biomes[j, i] =
            world.BiomeDataTable.GetBiome(biomeIds[(int)(0.861111f * j * 4), (int)(0.861111f * i * 4)]);
        }
      }
    ...
    private void GenerateBasicTerrain(IWorld world, ChunkPos pos, BlockData[,,] blocks, GenerationHelper helper,
      GenerationContext context)
    {
      ...
      var airBlock = world.BlockDataTable.GetBlock(AirBlock);
      var stoneBlock = world.BlockDataTable.GetBlock(StoneBlock);
      var waterBlock = world.BlockDataTable.GetBlock(WaterBlock);

      // 进行线性插值
      for (var xHigh = 0; xHigh < 4; xHigh++)
      {
        for (var zHigh = 0; zHigh < 4; zHigh++)
        {
          for (var yHigh = 0; yHigh < 32; yHigh++)
          {
            var yPart111 = context.DensityMap[xHigh, yHigh, zHigh];
            var yPart121 = context.DensityMap[xHigh, yHigh, zHigh + 1];
            var yPart211 = context.DensityMap[xHigh + 1, yHigh, zHigh];
            var yPart221 = context.DensityMap[xHigh + 1, yHigh, zHigh + 1];
            var yDensityStep11 = (context.DensityMap[xHigh, yHigh + 1, zHigh] - yPart111) * 0.125f;
            var yDensityStep12 = (context.DensityMap[xHigh, yHigh + 1, zHigh + 1] - yPart121) * 0.125f;
            var yDensityStep21 = (context.DensityMap[xHigh + 1, yHigh + 1, zHigh] - yPart211) * 0.125f;
            var yDensityStep22 = (context.DensityMap[xHigh + 1, yHigh + 1, zHigh + 1] - yPart221) * 0.125f;

            for (var yLow = 0; yLow < 8; yLow++)
            {
              var density111 = yPart111;
              var density121 = yPart121;
              var xDensityStep11 = (yPart211 - yPart111) * 0.25f;
              var xDensityStep21 = (yPart221 - yPart121) * 0.25f;

              for (var xLow = 0; xLow < 4; xLow++)
              {
                var zDensityStep11 = (density121 - density111) * 0.25f;
                var blockValue = density111 - zDensityStep11;

                for (var zLow = 0; zLow < 4; zLow++)
                {
                  var posX = xHigh * 4 + xLow;
                  var posY = yHigh * 8 + yLow;
                  var posZ = zHigh * 4 + zLow;
    ...
    {
      var depthOffset = new Vector3(noiseOffset.x + 0.1f, 0.0f, noiseOffset.z + 0.1f);
      var depthScale = new Vector3(DepthNoiseScale.x, 1.0f, DepthNoiseScale.z);
      helper.DepthNoise.Noise(context.DepthMap, depthOffset, depthScale);

      var noiseScale = new Vector3(CoordinateScale, HeightScale, CoordinateScale);
      ...
      for (var x1 = 0; x1 < 5; x1++)
      {
        for (var z1 = 0; z1 < 5; z1++)
        {
          ...
          // 中心点生物群系
          var centerBiome = context.Biomes[z1 + 2, x1 + 2];
          ...
          for (var x2 = 0; x2 < 5; x2++)
          {
            ...
            {
              var biome = context.Biomes[z1 + z2, x1 + x2];
              var curGroundYOffset = BiomeDepthOffset + biome.BaseHeight * BiomeDepthWeight; // biomeDepthOffSet = 0
              var curScale = BiomeScaleOffset + biome.HeightVariation * BiomeScaleWeight; // biomeScaleOffset = 0

              // parabolicField 为 10 / √(该点到中心点的距离^2 + 0.2)
              var weight = helper.BiomeWeights[z2, x2] / (curGroundYOffset + 2.0f);
          ...
          var random = (context.DepthMap[x1, 0, z1] - 0.5f) * 2 / 8000f;
          ...
          var groundYOffset1 = groundYOffset;
          var scale1 = scale;
          ...
          // 这个是大概的地面 y 坐标
          var groundY = BaseSize + groundYOffset1 * 4.0f; // baseSize = 8.5，应该代表了平均地表高度 68

          // 注意这个 y * 8 才是最终的y坐标
          for (var y = 0; y < 33; y++)
          {
            // result 偏移量，这个是负数则趋向固体，是正数则趋向液体和空气
            var offset = (y - groundY) * StretchY * 128.0f / 256.0f / scale1; // scale 大概在 0.1 ~ 0.2 这样...
            ...
            var lowerLimit =
              (context.MinLimitMap[x1, y, z1] - 0.5f) * 160000 / LowerLimitScale; // lowerLimitScale = 512
            var upperLimit =
              (context.MaxLimitMap[x1, y, z1] - 0.5f) * 160000 / UpperLimitScale; // upperLimitScale = 512
            var t = ((context.MainNoiseMap[x1, y, z1] - 0.5f) * 160000 / 10.0f + 1.0f) / 2.0f;

            // 这个函数 t < 0 则取 lowerLimit，t > 1 则取 upperLimit，否则以 t 为参数在上下限间线性插值
            var result = Mathf.Lerp(lowerLimit, upperLimit, t) - offset;

            // y = 30 ~ 32
            if (y > 29)
            {
              // 在原 result 和 -10 之间线性插值，这样 y > 240 的方块就会越来越少，最后全变成空气
              var t2 = (float)(y - 29) / 3f;
    ...
    private void ReplaceBiomeBlocks(IWorld world, ChunkPos pos, BlockData[,,] blocks, GenerationHelper helper,
      GenerationContext context)
    {
      helper.SurfaceNoise.Noise(context.SurfaceMap, new Vector3(pos.X + 0.1f, 0, pos.Z + 0.1f),
        new Vector3(0.0625f, 1f, 0.0625f));
    ...
    private void GenerateBiomeTerrain(int columnX, int columnZ, IWorld world, BlockData[,,] blocks,
      GenerationContext context)
    {
      var noise = (context.SurfaceMap[columnX, 0, columnZ] - 0.5f) * 2;
      var biome = context.Biomes[columnZ, columnX];

      var topBlock = world.BlockDataTable.GetBlock(biome.TopBlock);
      var fillerBlock = world.BlockDataTable.GetBlock(biome.FillerBlock);
      var airBlock = world.BlockDataTable.GetBlock(AirBlock);
      var stoneBlock = world.BlockDataTable.GetBlock(StoneBlock);
      var gravelBlock = world.BlockDataTable.GetBlock(GravelBlock);
      var bedrockBlock = world.BlockDataTable.GetBlock(BedrockBlock);

      var currentTopBlock = topBlock;
      var currentFillerBlock = fillerBlock;

      var surfaceFlag = -1;
      var surfaceDepth = (int)(noise / 3.0f + 3.0f + (float)context.Rand.NextDouble() * 0.25f);
      ...
      for (var y = ChunkHeight - 1; y >= 0; y--)
      {
        ...
        {
          var block = blocks[columnX, y, columnZ];
