...
namespace Minecraft.Scripts
{
  ...
  public class Chunk : IWorldRWAccessor, IDisposable
  {
    ...
    public Vector3Int WorldSpaceOrigin =>
      Accessible ? Position.XOZ : throw new InvalidOperationException("Chunk is not accessible.");
    ...
    public ChunkPos Position =>
      Accessible ? m_Position : throw new InvalidOperationException("Chunk is not accessible.");
    ...
    public void GetRawDataNoCheck(out ChunkPos pos, out IWorld world, out BlockData[,,] blocks,
      out Quaternion[,,] rotations, out NibbleArray skyLights, out byte[,] heightMap)
    {
      pos = m_Position;
      world = m_World;
      blocks = m_Blocks;
    ...
    private void UpdateSkyLightData(int x, int z, ModificationSource source)
    {
      ...
      {
        var index = GetNibbleArrayIndex(x, y, z);
    ...
    public bool SetBlock(int x, int y, int z, BlockData value, Quaternion rotation, ModificationSource source)
    {
      ...
      ref var blockData = ref m_Blocks[x, y, z];

      if (blockData != value)
      {
        var previousBlock = blockData;
        blockData = value;
        m_Rotations[x, y, z] = rotation;

        var topVisibleBlockY = GetTopVisibleBlockY(x, z);
        var visible = !value.HasFlag(BlockFlags.AlwaysInvisible);
    ...
    public int GetMixedLightLevel(int x, int y, int z, int defaultValue = 0)
    {
      ...
      ref var blockData = ref m_Blocks[x, y, z];
      var result = blockData.LightValue;
      var nIndex = GetNibbleArrayIndex(x, y, z);
