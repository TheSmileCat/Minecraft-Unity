...
namespace Minecraft.Scripts.PhysicSystem
{
  ...
  public static class Physics
  {
    ...
    /// <returns></returns>
    public static bool RaycastBlock(Ray ray, float maxDistance, IWorld world, Func<BlockData, bool> selectBlock,
      out BlockRaycastHit hit)
    {
      return RaycastBlock(ray.origin, ray.origin + ray.direction * maxDistance, world, selectBlock, out hit);
    }

    public static bool RaycastBlock(Vector3 origin, Vector3 direction, float maxDistance, IWorld world,
      Func<BlockData, bool> selectBlock, out BlockRaycastHit hit)
    {
      return RaycastBlock(origin, direction * maxDistance, world, selectBlock, out hit);
    }

    public static bool RaycastBlock(Vector3 start, Vector3 end, IWorld world, Func<BlockData, bool> selectBlock,
      out BlockRaycastHit hit)
    {
      var startX = Mathf.FloorToInt(start.x);
      var startY = Mathf.FloorToInt(start.y);
      var startZ = Mathf.FloorToInt(start.z);

      var endX = Mathf.FloorToInt(end.x);
      var endY = Mathf.FloorToInt(end.y);
      var endZ = Mathf.FloorToInt(end.z);

      var count = 200; // 上限200个方块，防止死循环
      ...
      while ((startX != endX || startY != endY || startZ != endZ) && (count-- > 0))
      {
        ...
        var xt = float.PositiveInfinity;
        var yt = float.PositiveInfinity;
        var zt = float.PositiveInfinity;
        ...
        // 检测新起点方块
        var pos = new Vector3Int(startX, startY, startZ);
        var block = world.RWAccessor.GetBlock(startX, startY, startZ);
    ...
    {
      aabb += position;
      var y = Mathf.FloorToInt(aabb.Min.y - 0.5f);

      if (y >= 0 && y < ChunkHeight)
      {
        var startX = Mathf.FloorToInt(aabb.Min.x);
        var endX = Mathf.FloorToInt(aabb.Max.x - 0.05f); // 稍微做一个偏移，避免误判
        var startZ = Mathf.FloorToInt(aabb.Min.z);
        var endZ = Mathf.FloorToInt(aabb.Max.z - 0.05f); // 稍微做一个偏移，避免误判
        ...
        for (var x = startX; x <= endX; x++)
        {
          for (var z = startZ; z <= endZ; z++)
          {
            ...
            var blockAABB = groundBlock.GetBoundingBox(x, y, z, world, true);
    ...
    public static CollisionFlags CollideWithTerrainAABB(Vector3 position, AABB aabb, IWorld world,
      PhysicMaterial material, float time, ref Vector3 velocity, out Vector3 movement)
    {
      aabb += position;
    ...
    private static void CalculateVelocityAfterCollisionWithTerrain(PhysicMaterial material, BlockData blockCollided,
      ref float velocity)
    {
      // 碰撞公式：
      ...
      var e = blockCollided.PhysicMaterial.CoefficientOfRestitution;
    ...
    private static bool CollideWithTerrainAABB(int axis, AABB aabb, IWorld world, PhysicMaterial material, float time,
      ref float velocity, out float movement)
    {
      ...
      for (var x = start.x; x != end.x; x += step.x)
      {
        for (var z = start.z; z != end.z; z += step.z)
        {
          for (var y = start.y; y != end.y; y += step.y)
          {
            var block = world.RWAccessor.GetBlock(x, y, z);
            var blockAABB = block.GetBoundingBox(x, y, z, world, true);
            ...
            {
              var maxMovement = blockBB.Min[axis] - aabb.Max[axis];
            ...
            {
              var maxMovement = blockBB.Max[axis] - aabb.Min[axis];
