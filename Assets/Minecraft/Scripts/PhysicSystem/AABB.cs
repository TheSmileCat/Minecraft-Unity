...
namespace Minecraft.Scripts.PhysicSystem
{
  ...
  public struct AABB : ILuaCallCSharp
  {
    ...
    {
      var step = aabb.Max - aabb.Min;
      var min = Vector3.positiveInfinity;
      var max = Vector3.negativeInfinity;

      for (var x = aabb.Min.x; x <= aabb.Max.x; x += step.x)
      {
        for (var y = aabb.Min.y; y <= aabb.Max.y; y += step.y)
        {
          for (var z = aabb.Min.z; z <= aabb.Max.z; z += step.z)
          {
            var point = MathUtility.RotatePoint(new Vector3(x, y, z), rotation, pivot);
