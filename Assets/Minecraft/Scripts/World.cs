...
namespace Minecraft.Scripts
{
  ...
  public abstract class World : MonoBehaviour, IWorld
  {
    ...
    [Header("Asset Names")] [SerializeField]
    private string m_BlockTableAssetName;
    ...
    [Space] [Header("Local Player References")] [SerializeField]
    private Transform m_Player;

    [SerializeField] private Camera m_MainCamera;

    [Space] [Header("Manager References")] [SerializeField]
    ...
    [Header("Others")] [SerializeField] private int m_Seed = 0;
    ...
    [field: NonSerialized] public BlockTable BlockDataTable { get; private set; }

    [field: NonSerialized] public BiomeTable BiomeDataTable { get; private set; }

    [field: NonSerialized] public WorldGeneratePipeline WorldGenPipeline { get; private set; }
    ...
    private IEnumerator Start()
    {
      ...
      var blockTable = AssetManager.Instance.LoadAsset<BlockTable>(m_BlockTableAssetName);
      var biomeTable = AssetManager.Instance.LoadAsset<BiomeTable>(m_BiomeTableAssetName);
      var worldGenPipeline = AssetManager.Instance.LoadAsset<WorldGeneratePipeline>(m_WorldGenPipelineAssetName);

      yield return blockTable;
      BlockDataTable = blockTable.GetAssetAs<BlockTable>();
      yield return BlockDataTable.Initialize();

      yield return biomeTable;
      BiomeDataTable = biomeTable.GetAssetAs<BiomeTable>();
      yield return BiomeDataTable.Initialize();

      yield return worldGenPipeline;
      WorldGenPipeline = worldGenPipeline.GetAssetAs<WorldGeneratePipeline>();
    ...
    [Serializable]
    private class OnInitializedEvent : UnityEvent<IWorld>
    {
    }
    ...
    private class DefaultWorldRWAccessor : IWorldRWAccessor
    {
      ...
      {
        var pos = ChunkPos.GetFromAny(x, z);

        if (World.ChunkManager.GetChunk(pos, false, out var chunk))
        {
