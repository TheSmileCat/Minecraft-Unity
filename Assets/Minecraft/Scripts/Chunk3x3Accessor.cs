...
namespace Minecraft.Scripts
{
  public class Chunk3x3Accessor : IWorldRAccessor, IEnumerable<Chunk>, IDisposable
  {
    ...
    {
      for (var x = XOffsetBegin; x <= XOffsetEnd; x++)
      {
        for (var z = ZOffsetBegin; z <= ZOffsetEnd; z++)
        {
          var neighbor = pos.AddOffset(x, z);
          var result = chunkManager.GetChunk(neighbor, false, out var chunk);
    ...
    private Chunk GetChunk(ref int x, ref int z)
    {
      ...
      var xOffset = Mathf.FloorToInt((float)x / ChunkWidth);
      var zOffset = Mathf.FloorToInt((float)z / ChunkWidth);
    ...
    {
      var chunk = GetChunk(ref x, ref z);
    ...
    {
      for (var x = 0; x < m_Members.GetLength(0); x++)
      {
        for (var z = 0; z < m_Members.GetLength(1); z++)
        {
