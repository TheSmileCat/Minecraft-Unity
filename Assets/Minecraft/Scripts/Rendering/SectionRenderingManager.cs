...
namespace Minecraft.Scripts.Rendering
{
  ...
  public class SectionRenderingManager : MonoBehaviour, ILuaCallCSharp
  {
    ...
    {
      var radius = m_Setting.RenderingRadius;
      var width = radius * 2 + 1;
      var length = width * width * SectionCountInChunk;
      ...
      for (var x = -radius; x <= radius; x++)
      {
        ...
        {
          for (var i = 0; i < SectionCountInChunk; i++)
          {
    ...
    public void MarkBlockMeshDirty(int x, int y, int z, ModificationSource source)
    {
      ...
      var forceLoadMesh = source == ModificationSource.PlayerAction;

      // 被标记为 important 后，会在主线程进行更新，量大的话容易卡
      var important = source == ModificationSource.PlayerAction;
    ...
    private void CalculateSectionOffset()
    {
      ...
      var player = m_World.PlayerTransform.position;
      var playerChunk = ChunkPos.GetFromAny(player.x, player.z);
    ...
    private Mesh FindSectionMesh(int sectionIndex, out Vector3Int position)
    {
      ...
      var section = m_Sections[sectionIndex] + m_SectionOffset;
      position = new Vector3Int(section.x, section.y, section.z);
      var result = m_MeshManager.GetSectionMesh(position);
      Profiler.EndSample();
      return result;
    }
    ...
    private void RenderMeshes()
    {
      ...
      var subMeshCount = m_BlockTable.MaterialCount;
      ...
      var material = m_BlockTable.GetMaterial(0);

      for (var i = 0; i < m_VisibleSectionIndices.Length; i++)
      {
        var mesh = FindSectionMesh(m_VisibleSectionIndices[i], out var pos);
        ...
        RenderSectionMesh(mesh, 0, pos, material, m_TargetLayer, m_World.MainCamera, m_Setting.CastShadows,
          m_Setting.ReceiveShadows);
      }
      ...
      for (var i = 1; i < subMeshCount; i++)
      {
        ...
        for (var j = 0; j < m_RenderMeshBuffer.Count; j++)
        {
          var (mesh, pos) = m_RenderMeshBuffer[j];
          RenderSectionMesh(mesh, i, pos, material, m_TargetLayer, m_World.MainCamera, m_Setting.CastShadows,
            m_Setting.ReceiveShadows);
        }
      }
    ...
    private void RenderSectionMesh(Mesh mesh, int subMeshIndex, Vector3 position, Material material, int layer,
      Camera camera, ShadowCastingMode castShadows, bool receiveShadows)
    {
      ...
      {
        Graphics.DrawMesh(mesh, matrix, material, layer, camera, subMeshIndex, null, castShadows, receiveShadows, null,
          false);
      }
