...
namespace Minecraft.Scripts.Rendering
{
  public abstract class MeshBuilder<TVertex, TIndex> : ILuaCallCSharp where TVertex : unmanaged where TIndex : unmanaged
  {
    ...
    protected unsafe MeshBuilder(VertexAttributeDescriptor[] vertexAttributes, int subMeshCount)
    {
      MeshIndexFormat = sizeof(TIndex) switch
      {
        ...
        _ => throw new NotSupportedException(
          $"The index type '{typeof(TIndex)}' is not supported! The size of the type must be 2 or 4.")
      };
    ...
    public unsafe void AddIndex(int index, int subMesh = 0)
    {
      ...
      for (var i = subMesh + 1; i < m_IndexOffsetBuffer.Length; i++)
      {
    ...
    public Bounds CalculateObjectSpaceBounds()
    {
      ...
      for (var i = 0; i < m_VertexBuffer.Count; i++)
      {
        var pos = GetPositionOS(m_VertexBuffer[i]);
    ...
    public void ApplyAndClearBuffers(Mesh.MeshData meshData, MeshTopology topology)
    {
      ...
      {
        var subMesh = GetSubMeshDescriptor(i, topology);
        meshData.SetSubMesh(i, subMesh, flags);
      }

      // vertex
      var vertices = meshData.GetVertexData<TVertex>();
      for (var i = 0; i < m_VertexBuffer.Count; i++)
      {
      ...
      // index
      var indices = meshData.GetIndexData<TIndex>();
      for (var i = 0; i < m_IndexBuffer.Count; i++)
      {
    ...
    public void ApplyAndClearBuffers(Mesh mesh, MeshTopology topology, bool markNoLongerReadable, Allocator allocator)
    {
      ...
      // index
      var indices = new NativeArray<TIndex>(m_IndexBuffer.Count, allocator, NativeArrayOptions.UninitializedMemory);
      for (var i = 0; i < m_IndexBuffer.Count; i++)
      {
        CopyMeshIndex(indices, m_IndexBuffer[i]);
      }
    ...
    {
      var nextSubMesh = subMesh + 1;
      var offset = m_IndexOffsetBuffer[subMesh];
      var indexCount = (nextSubMesh < SubMeshCount ? m_IndexOffsetBuffer[nextSubMesh] : m_IndexBuffer.Count) - offset;
