...
namespace Minecraft.Scripts.Rendering
{
  ...
  public class SectionMeshManager : MonoBehaviour, ILuaCallCSharp
  {
    ...
    private void Update()
    {
      ...
      var position = m_World.PlayerTransform.position;
      var limit = m_BuildNotImportantMeshPreFrame;

      while (m_NotImportantDirtySectionQueue.Count > 0 && limit-- > 0)
      {
        var section = m_NotImportantDirtySectionQueue.Dequeue();
      ...
      {
        var section = m_ImportantDirtySectionQueue.Dequeue();
    ...
    private void TryEnqueueMeshWork(Vector3Int section, Vector3 playerPos, bool important)
    {
      ...
      var chunk = ChunkPos.Get(section.x, section.z);

      if (m_World.ChunkManager.GetChunk3x3Accessor(chunk, true, out var accessor))
      {
        m_DirtySections.Remove(section);
        var mesh = GetSectionMesh(section);
        ...
        else
        {
          ...
          var sectionXZ = new Vector2(section.x, section.z);
          var sqrDistance = (playerXZ - sectionXZ).sqrMagnitude;
          var deltaY = Mathf.Abs(playerPos.y - section.y);
    ...
    private void UpdateFallbacks(Queue<Vector3Int> queue)
    {
      ...
      for (var i = 0; i < m_DirtySectionFallbacks.Count; i++)
      {
    ...
    {
      if (m_SectionMeshMap.TryGetValue(section, out var mesh))
      {
