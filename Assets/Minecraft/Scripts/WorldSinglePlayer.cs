...
namespace Minecraft.Scripts
{
  internal class WorldSinglePlayer : World
  {
    ...
    private void LightBlocks()
    {
      ...
      var limit = MaxLightBlockCountPerFrame; // 防止卡死
    ...
    private void LightBlocks(Stack<Vector3Int> queue, ref int limit, ModificationSource source)
    {
      while (limit-- > 0 && queue.Count > 0)
      {
        ...
        var block = RWAccessor.GetBlock(x, y, z);
        var opacity = Mathf.Max(block.LightOpacity, 1);
        var finalLight = 0;

        if (opacity < MaxLight || block.LightValue > 0) // 不然就是0
        {
          var max = RWAccessor.GetAmbientLight(x + 1, y, z);
          int temp;
    ...
    {
      var count = m_BlocksToTickQueue.Count;
      ...
      while (count-- > 0)
      {
        var blockPos = m_BlocksToTickQueue.Dequeue();
        var x = blockPos.x;
        var y = blockPos.y;
        var z = blockPos.z;
