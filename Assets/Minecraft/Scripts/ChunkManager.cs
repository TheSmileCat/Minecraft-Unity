...
namespace Minecraft.Scripts
{
  ...
  {
    [Serializable]
    private class ChunkEvent : UnityEvent<Chunk>
    {
    }

    [Serializable]
    private class ChunkPosEvent : UnityEvent<ChunkPos>
    {
    }
    ...
    private void OnWillUnloadChunk(ChunkPos key, Chunk value)
    {
      ...
      for (var x = Chunk3x3Accessor.XOffsetBegin; x <= Chunk3x3Accessor.XOffsetEnd; x++)
      {
        for (var z = Chunk3x3Accessor.ZOffsetBegin; z <= Chunk3x3Accessor.ZOffsetEnd; z++)
        {
          var neighbor = key.AddOffset(x, z);
    ...
    public bool GetChunk3x3Accessor(ChunkPos pos, bool load, out Chunk3x3Accessor accessor)
    {
      ...
      var result = load && LoadChunk3x3Accessor(pos, out accessor);
      Profiler.EndSample();
      return result;
    }
