...
namespace Minecraft.Scripts.Lua
{
  public class LuaManager : MonoBehaviour, ILuaCallCSharp
  {
    ...
    public IEnumerator Initialize()
    {
      ...
      for (var i = 0; i < m_LuaAssetBundles.Length; i++)
      {
      ...
      for (var i = 0; i < m_LuaFileAssets.Count; i++)
      {
        var asset = m_LuaFileAssets[i];
    ...
    {
      var fileName = $"{filepath.Replace('.', '/')}.lua";

      for (var i = 0; i < m_SearchPaths.Length; i++)
      {
        filepath = $"{m_SearchPaths[i]}/{fileName}";

        if (m_LuaFileMap.TryGetValue(filepath, out var asset))
        {
