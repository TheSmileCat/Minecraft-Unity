...
namespace Minecraft.Scripts.Collections
{
  public class LRULinkedMap<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>, ILuaCallCSharp
  {
    ...
    public TValue this[TKey key]
    {
      get
      {
        var node = FindNode(key) ?? throw new KeyNotFoundException();
        return node.Value;
      }
    ...
    private LRUNode FindNode(TKey key)
    {
      ...
      var hashCode = Comparer.GetHashCode(key) & int.MaxValue; // <=> abs(hash)
      var node = m_Buckets[hashCode % m_Buckets.Length];
    ...
    public void AddOrSet(TKey key, TValue value)
    {
      ...
      var hashCode = Comparer.GetHashCode(key) & int.MaxValue; // <=> abs(hash)
      var index = hashCode % m_Buckets.Length;
