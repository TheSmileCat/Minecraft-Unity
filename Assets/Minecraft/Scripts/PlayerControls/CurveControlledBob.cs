...
namespace Minecraft.Scripts.PlayerControls
{
  ...
  public class CurveControlledBob
  {
    ...
    [SerializeField] private Vector2 m_BobAmplitude = new(0.1f, 0.1f);

    [SerializeField] private AnimationCurve m_BobCurve = new(
      new Keyframe(0f, 0f),
    ...
    {
      var xPos = m_OriginalCameraPosition.x + (m_BobCurve.Evaluate(m_CyclePosition.x) * m_BobAmplitude.x);
      var yPos = m_OriginalCameraPosition.y + (m_BobCurve.Evaluate(m_CyclePosition.y) * m_BobAmplitude.y);
