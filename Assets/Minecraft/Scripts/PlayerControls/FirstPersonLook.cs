...
namespace Minecraft.Scripts.PlayerControls
{
  ...
  public class FirstPersonLook
  {
    ...
    [Space] public bool ClampVerticalRotation = true;
    public Vector2 Sensitivity = new(0.2f, 0.2f);
    [MinMaxSlider(-180, 180)] public Vector2 AngleRangeX = new(-90f, 90f);
    ...
    public void LookRotation(Vector2 rotation, float deltaTime)
    {
      ...
      if (EnableSmooth)
      {
        m_Character.localRotation =
          Quaternion.Slerp(m_Character.localRotation, m_CharacterRotation, SmoothTime * deltaTime);
    ...
    private void ClampRotationAroundXAxis(ref Quaternion q)
    {
      ...
      var angleX = Mathf.Clamp(2.0f * Mathf.Rad2Deg * Mathf.Atan(q.x), AngleRangeX.x, AngleRangeX.y);
