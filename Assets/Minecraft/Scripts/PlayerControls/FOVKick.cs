...
namespace Minecraft.Scripts.PlayerControls
{
  ...
  {
    [SerializeField] [Tooltip("the amount the field of view increases when going into a run")]
    private float m_FOVIncrease = 3f;

    [SerializeField] [Tooltip("the amount of time the field of view will increase over")]
    private float m_TimeToIncrease = 1f;

    [SerializeField] [Tooltip("the amount of time the field of view will take to return to its original size")]
    private float m_TimeToDecrease = 1f;

    [SerializeField] private AnimationCurve m_IncreaseCurve;
    ...
    {
      var t = Mathf.Abs((m_Camera.fieldOfView - m_OriginalFov) / m_FOVIncrease);
