...
namespace Minecraft.Scripts.PlayerControls
{
  ...
  public class BlockInteraction : MonoBehaviour, ILuaCallCSharp
  {
    ...
    private void Update()
    {
      ...
      var ray = GetRay();
      var world = m_PlayerEntity.World;
      DigBlock(ray, world);
    ...
    private bool ChangeHandBlock()
    {
      ...
      if (m_HandBlockInputGO.activeInHierarchy)
      {
        ...
        for (var i = 0; i < m_DisableWhenEditHandBlock.Length; i++)
        {
    ...
    {
      if (Physics.RaycastBlock(ray, RaycastMaxDistance, world, m_DestroyRaycastSelector, out var hit))
      {
        ...
        if (Input.GetMouseButton(0))
        {
          if (m_IsDigging)
          {
            if (hit.Position == m_FirstDigPos)
            {
              ...
              if (m_DiggingDamage >= hit.Block.Hardness)
              {
                SetDigProgress(0);
                m_IsDigging = false;
                world.RWAccessor.SetBlock(hit.Position.x, hit.Position.y, hit.Position.z,
                  world.BlockDataTable.GetBlock(0), Quaternion.identity, ModificationSource.PlayerAction);
          ...
          {
            var block = world.RWAccessor.GetBlock(hit.Position.x, hit.Position.y, hit.Position.z);
            m_IsDigging = true;
            m_DiggingDamage = 0;
    ...
    private void PlaceBlock(Ray ray, IWorld world)
    {
      ...
      {
        var block = world.BlockDataTable.GetBlock(m_CurrentHandBlockText.text);

        if (Physics.RaycastBlock(ray, RaycastMaxDistance, world, m_PlaceRaycastSelector, out var hit))
        {
          var pos = (hit.Position + hit.Normal).FloorToInt();
          var playerBB = m_PlayerEntity.BoundingBox + m_PlayerEntity.Position;
          var blockBB = hit.Block.GetBoundingBox(pos, world, false).Value;
          ...
          if (!playerBB.Intersects(blockBB))
          {
            ...
            {
              var forward = m_PlayerEntity.Forward;
